...
//	@SuppressLint("StaticFieldLeak")
	...
	public static class ValiFiConfig {
		...
		ValiFiConfig(ValiFieldBase.PropertyValidator<String>[] validators, int[] errorResources, long errorDelay, long asyncValidationDelay, @NotNull Set<ValiFiCardType> knownCardTypes) {
...
//	 * @see #setPattern(int, Pattern)
	...
	public static class Builder {
		...
		private void setupPatterns() {
			...
			setPattern(PATTERN.PATTERN_EMAIL, Pattern.compile("[a-zA-Z0-9+._%\\-]{1,256}" + "@" + "[a-zA-Z0-9][a-zA-Z0-9\\-]{0,64}" + "(" + "\\." + "[a-zA-Z0-9][a-zA-Z0-9\\-]{0,25}" + ")+"));
			setPattern(PATTERN.PATTERN_PHONE, Pattern.compile("^\\+420 ?[1-9][0-9]{2} ?[0-9]{3} ?[0-9]{3}$" + "|" + "^(\\+?1)?[2-9]\\d{2}[2-9](?!11)\\d{6}$"));            // phone czech | phone en-US
			setPattern(PATTERN.PATTERN_USERNAME, Pattern.compile(".{4,}"));
			setPattern(PATTERN.PATTERN_PASSWORD, Pattern.compile(".{8,}"));
		}

		private void setupResources() {
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_NOT_EMPTY)] = ResourceTable.String_validation_error_empty;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_LENGTH_MIN)] = ResourceTable.String_validation_error_min_length;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_LENGTH_MAX)] = ResourceTable.String_validation_error_max_length;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_LENGTH_RANGE)] = ResourceTable.String_validation_error_range_length;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_LENGTH_EXACT)] = ResourceTable.String_validation_error_exact_length;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_EMAIL)] = ResourceTable.String_validation_error_email;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_PHONE)] = ResourceTable.String_validation_error_phone;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_USERNAME)] = ResourceTable.String_validation_error_username;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_PASSWORD)] = ResourceTable.String_validation_error_password;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_YEARS_OLDER_THAN)] = ResourceTable.String_validation_error_older_than_years;
			mErrorResources[getErrorResNum(ERROR_RES.ERROR_RES_CREDIT_CARD)] = ResourceTable.String_validation_error_credit_card;
		}
	}
}
